    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      ...
      <main className="relative max-w-7xl mx-auto px-6 py-8">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <span className={`inline-block px-3 py-1.5 rounded-lg font-semibold text-sm ${
                        aiAnalysis.recommendedAction === 'BLOCK' ? 'bg-red-500/20 text-red-400' :
          ...
          <div className="space-y-6">
            ...
            {batchClaims.length > 0 ? (
              <div className="p-6 rounded-2xl bg-slate-800/30 border border-slate-700/50">
                ...
                <div className="grid grid-cols-12 gap-2 h-48 items-end">
                  {Array.from({ length: 12 }, (_, month) => {
                    const monthClaims = batchClaims.filter(c => {
                      const d = new Date(c.dateOfService);
                      return d.getMonth() === month;
                    });
                    const flagged = monthClaims.filter(c => !analyzeClaimWithRules(c).isClean).length;
                    const total = monthClaims.length;
                    const height = total > 0 ? (total / 60) * 100 : 0;
                    const flaggedHeight = flagged > 0 ? (flagged / 60) * 100 : 0;
                    
                    return (
                      <div key={month} className="flex flex-col items-center">
                        <div className="w-full relative" style={{ height: `${height}%`, minHeight: total > 0 ? '8px' : '0' }}>
                          <div className="absolute bottom-0 w-full bg-cyan-500/30 rounded-t" style={{ height: '100%' }} />
                          <div className="absolute bottom-0 w-full bg-red-500/50 rounded-t" style={{ height: `${(flaggedHeight / height) * 100}%` }} />
                        </div>
              ...
              <div className="text-center py-16">
                ...
                <p className="text-slate-400">Run a batch analysis first to generate temporal data</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'investigator' && (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-semibold text-slate-200">AI Fraud Investigator</h2>
              <p className="text-slate-400 mt-1">Ask questions about the analyzed data in plain English</p>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-violet-500/10 to-cyan-500/10 border border-violet-500/30">
              <div className="flex gap-4">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-xl bg-slate-900/50 border border-slate-700/50 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-violet-500/50"
                />
                <button
                  onClick={askInvestigator}
                  disabled={isInvestigating || !investigatorQuery.trim()}
                  className="px-6 py-3 rounded-xl bg-violet-500 text-white font-medium hover:bg-violet-600 transition-colors disabled:opacity-50 flex items-center gap-2"
                >
                  {isInvestigating ? (
                    <Loader2 className="w-5 h-5 animate-spin" />
                  ) : (
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-slate-800/50 text-sm text-slate-400 hover:text-slate-200 hover:bg-slate-700/50 transition-colors"
                  >
                    {q}
                  </button>
                ))}
            ...
            {investigatorResponse && (
              <div className="p-6 rounded-2xl bg-slate-800/30 border border-slate-700/50">
                ...
                <div className="prose prose-invert prose-sm max-w-none">
                  <pre className="whitespace-pre-wrap text-slate-300 text-sm font-sans leading-relaxed">{investigatorResponse}</pre>
                </div>
              </div>
            )}
          </div>
