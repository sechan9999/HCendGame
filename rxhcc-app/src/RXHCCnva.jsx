...
import { AlertTriangle, Shield, Search, Activity, Network, Clock, Bot, ChevronRight, Check, X, Loader2, Settings, Play, BarChart3, Users, DollarSign, FileText, Zap, Brain, TrendingUp, FlaskConical, GitBranch, RefreshCw, TrendingDown, ChevronDown, Download } from 'lucide-react';
...
const AUTORESEARCH_EXPERIMENTS = [
  {
    id: 'baseline',
    commit: 'a0f3e21',
    description: 'Baseline rule engine (NDC-Dx mismatch + HCC accumulation + duplicate NDC)',
    f1: 0.760, precision: 0.820, recall: 0.710,
    memoryMb: 0, status: 'keep', delta: null,
    ruleAdded: null,
    log: 'Baseline established. 3 core rules active. Known gap: doctor-shopping not caught.',
  },
  {
    id: 'exp01',
    commit: 'b1c4d35',
    description: 'Add specialty-mismatch rule: oncology drug billed by non-oncologist',
    f1: 0.791, precision: 0.851, recall: 0.740,
    memoryMb: 0.1, status: 'keep', delta: +0.031,
    ruleAdded: 'SPECIALTY_MISMATCH',
    log: 'F1 improved +0.031. Caught 4 new Keytruda fraud cases. No new FP introduced. Keeping.',
  },
  {
    id: 'exp02',
    commit: 'c2d5e46',
    description: 'Tighten HCC threshold from >1.5 to >1.2 to catch more upcoding',
    f1: 0.775, precision: 0.790, recall: 0.762,
    memoryMb: 0, status: 'discard', delta: -0.016,
    ruleAdded: 'HCC_THRESHOLD_TIGHTEN',
    log: 'F1 dropped -0.016. Precision fell due to 12 new false positives on legit multi-morbid patients. Reverting.',
  },
  {
    id: 'exp03',
    commit: 'd3e6f57',
    description: 'Add dual GLP-1 billing rule: same NDC class prescribed twice in one encounter',
    f1: 0.808, precision: 0.863, recall: 0.760,
    memoryMb: 0.2, status: 'keep', delta: +0.017,
    ruleAdded: 'DUAL_GLP1_BILLING',
    log: 'F1 improved +0.017. Caught 6 new duplicate-class billing cases. Simple 8-line rule. Keeping.',
  },
  {
    id: 'exp04',
    commit: 'e4f7g68',
    description: 'Add December HCC spike detection (MA risk score gaming)',
    f1: 0.808, precision: 0.863, recall: 0.760,
  ...
  },
  {
    id: 'exp05',
    commit: 'f5g8h79',
    description: 'Add doctor-shopping: patient with >2 providers in same drug class within 30 days',
  ...
  },
  {
    id: 'exp06',
    commit: 'g6h9i80',
    description: 'Add place-of-service mismatch: IV drug billed in office setting',
    f1: 0.849, precision: 0.878, recall: 0.821,
    memoryMb: 0.1, status: 'keep', delta: +0.015,
    ruleAdded: 'POS_MISMATCH',
    log: 'F1 improved +0.015. Precision+recall both up. Simple lookup rule. Keeping.',
  },
  {
    id: 'exp07',
    commit: 'h7i0j91',
    description: 'Add compound rule: high-cost drug + zero E&M code + new patient (cascade billing)',
    f1: 0.852, precision: 0.880, recall: 0.825,
...
export default function RxHCCFraudDetection() {
  ...
  const [arVisible, setArVisible] = useState(AUTORESEARCH_EXPERIMENTS.slice(0, 3));
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex gap-1">
            {[
              ...
              { id: 'autoresearch', label: 'AutoResearch', icon: FlaskConical, accent: 'violet' },
            ].map(tab => (
              <button
                key={tab.id}
              ...
              >
                ...
                {tab.label}
                {tab.id === 'autoresearch' && (
                  <span className="ml-1 px-1.5 py-0.5 rounded-full bg-violet-500/20 text-violet-400 text-xs font-bold">NEW</span>
                )}
              </button>
            ))}
          </div>
      ...
      <main className="relative max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="space-y-6">
            ...
            {batchResults && (
              <>
                ...
                {batchAiInsights && (() => {
                  let parsed = null;
                  try { parsed = JSON.parse(batchAiInsights); } catch (_) {}
                  const pct = parsed?.fraudProbability != null ? Math.round(parsed.fraudProbability * 100) : null;
                  return (
                    <div className="p-6 rounded-2xl bg-gradient-to-br from-violet-500/10 to-cyan-500/10 border border-violet-500/30">
                      <h3 className="text-lg font-semibold text-slate-200 mb-5 flex items-center gap-2 flex-wrap">
                        <Brain className="w-5 h-5 text-violet-400" />
                        AI Pattern Analysis
                        {parsed?.riskLevel && (
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${getRiskColor(parsed.riskLevel)}`}>
                            {parsed.riskLevel} RISK
                          </span>
                        )}
                        {parsed?.recommendedAction && (
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${
                            parsed.recommendedAction === 'BLOCK' ? 'bg-red-500/20 text-red-300 border-red-500/40' :
                            parsed.recommendedAction === 'REVIEW' ? 'bg-amber-500/20 text-amber-300 border-amber-500/40' :
                            'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                          }`}>→ {parsed.recommendedAction}</span>
                        )}
                      </h3>
                      {parsed ? (
                        <div className="space-y-4">
                          {pct != null && (
                            <div className="p-4 rounded-xl bg-slate-900/50">
                              <div className="flex justify-between items-center mb-2">
                                ...
                                <span className={`text-xl font-bold ${pct >= 70 ? 'text-red-400' : pct >= 40 ? 'text-amber-400' : 'text-emerald-400'}`}>{pct}%</span>
                              </div>
                              <div className="h-2.5 rounded-full bg-slate-700/60 overflow-hidden">
                                <div className={`h-full rounded-full transition-all duration-700 ${pct >= 70 ? 'bg-gradient-to-r from-red-600 to-red-400' : pct >= 40 ? 'bg-gradient-to-r from-amber-600 to-amber-400' : 'bg-gradient-to-r from-emerald-600 to-emerald-400'}`}
                                  style={{ width: `${pct}%` }} />
                              </div>
                              <div className="flex justify-between text-xs text-slate-600 mt-1"><span>Low</span><span>Medium</span><span>High</span></div>
                            </div>
                          )}
                          {parsed.reasoning && (
                            <div className="p-4 rounded-xl bg-slate-900/50">
                              <h4 className="text-xs font-semibold text-violet-400 uppercase tracking-wider mb-2">AI Reasoning</h4>
                              <p className="text-slate-200 text-sm leading-relaxed">{parsed.reasoning}</p>
                            </div>
                          )}
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {parsed.clinicalEvidence?.length > 0 && (
                              <div className="p-4 rounded-xl bg-slate-900/50">
                                <h4 className="text-xs font-semibold text-cyan-400 uppercase tracking-wider mb-2">Clinical Evidence</h4>
                                <ul className="space-y-1.5">
                                  {parsed.clinicalEvidence.map((e, i) => (
                                    <li key={i} className="flex items-start gap-2 text-sm text-slate-300">
                                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 mt-1.5 shrink-0" />{e}
                                    </li>
                                  ))}
                                </ul>
                              </div>
                            ...
                            {parsed.suggestedInvestigation && (
                              <div className="p-4 rounded-xl bg-slate-900/50 border border-amber-500/20">
                                <h4 className="text-xs font-semibold text-amber-400 uppercase tracking-wider mb-2">Suggested Actions</h4>
                                <p className="text-slate-300 text-sm leading-relaxed">{parsed.suggestedInvestigation}</p>
                              </div>
                            )}
                          </div>
                        </div>
                      ) : (
                        <p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{batchAiInsights}</p>
                      )}
                    </div>
                  );
                })()}
        ...
        {activeTab === 'autoresearch' && (() => {
          const kept = arVisible.filter(e => e.status === 'keep');
          const bestF1 = Math.max(...kept.map(e => e.f1));
          const bestExp = kept.find(e => e.f1 === bestF1);
          const allKept = arVisible.filter(e => e.status === 'keep');
          const f1Points = arVisible.map(e => ({ ...e }));
          const maxF1 = 0.92; const minF1 = 0.74;
          const chartW = 560; const chartH = 140;
          const toX = (i) => 28 + i * ((chartW - 56) / Math.max(f1Points.length - 1, 1));
          const toY = (v) => chartH - 16 - ((v - minF1) / (maxF1 - minF1)) * (chartH - 32);
          const polyPts = f1Points.map((e, i) => `${toX(i)},${toY(e.f1)}`).join(' ');

          const nextIdx = arCurrentIdx + 1;
          const nextExp = nextIdx < AUTORESEARCH_EXPERIMENTS.length ? AUTORESEARCH_EXPERIMENTS[nextIdx] : null;
          ...
          return (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: 'Experiments Run', value: arVisible.length, color: 'text-violet-400', bg: 'bg-violet-500/20' },
                  { label: 'Best F1 Score', value: bestF1.toFixed(3), color: 'text-emerald-400', bg: 'bg-emerald-500/20' },
                  { label: 'Rules Kept', value: arVisible.filter(e => e.status === 'keep').length, color: 'text-cyan-400', bg: 'bg-cyan-500/20' },
                  { label: 'Discarded', value: arVisible.filter(e => e.status === 'discard').length, color: 'text-amber-400', bg: 'bg-amber-500/20' },
                ].map((k, i) => (
                  <div key={i} className="p-4 rounded-2xl bg-slate-800/30 border border-slate-700/50 flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full ${k.bg.replace('/20', '')} animate-pulse`} />
                    <div>
              ...
              <div className="p-6 rounded-2xl bg-slate-800/30 border border-slate-700/50">
                ...
                <div className="overflow-x-auto">
                  <svg width={chartW} height={chartH + 30} className="min-w-full" style={{ minWidth: chartW }}>
                    {/* Grid */}
                    {[0.76, 0.80, 0.84, 0.88].map(v => (
                      <g key={v}>
                        <line x1={20} y1={toY(v)} x2={chartW - 8} y2={toY(v)} stroke="#334155" strokeWidth={1} strokeDasharray="4 3" />
                        <text x={16} y={toY(v) + 4} fill="#64748b" fontSize={9} textAnchor="end">{v.toFixed(2)}</text>
                      </g>
                    ))}
                    ...
                    <polygon
                      points={`${toX(0)},${chartH - 16} ${polyPts} ${toX(f1Points.length - 1)},${chartH - 16}`}
                      fill="url(#f1grad)"
                    />
                    {/* Line */}
                    <polyline points={polyPts} fill="none" stroke="#8b5cf6" strokeWidth={2.5} strokeLinejoin="round" />
                    {/* Points */}
                    {f1Points.map((e, i) => (
                      <g key={i}>
                        <circle cx={toX(i)} cy={toY(e.f1)} r={5}
                          fill={e.status === 'keep' ? '#8b5cf6' : e.status === 'discard' ? '#f59e0b' : '#64748b'}
                          stroke="#0f172a" strokeWidth={2} />
                        <text x={toX(i)} y={chartH + 22} textAnchor="middle" fill="#64748b" fontSize={9}>
                          {e.id === 'baseline' ? 'base' : e.id.replace('exp', 'E')}
                        </text>
                      </g>
                    ))}
                    {/* Best marker */}
                ...
                <div className="flex items-center gap-6 mt-2 text-xs text-slate-500">
                  <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-violet-500" /> Kept</div>
                  <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-amber-500" /> Discarded</div>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm font-mono">
                    <thead>
                      <tr className="text-left text-slate-500 border-b border-slate-700/50">
                        <th className="pb-2 pr-3 font-normal">commit</th>
                        <th className="pb-2 pr-3 font-normal text-right">f1</th>
                        <th className="pb-2 pr-3 font-normal text-right">precision</th>
                        <th className="pb-2 pr-3 font-normal text-right">recall</th>
                        <th className="pb-2 pr-3 font-normal text-right">delta</th>
                    ...
                    <tbody>
                      {arVisible.map((e, i) => (
                        <tr key={e.id} className={`border-b border-slate-800/40 transition-all ${i === arVisible.length - 1 ? 'bg-violet-500/5' : ''}`}>
                          <td className="py-2 pr-3 text-slate-500 text-xs">{e.commit}</td>
                          <td className="py-2 pr-3 text-right font-semibold text-slate-200">{e.f1.toFixed(4)}</td>
                          ...
                          <td className="py-2 text-slate-400 text-xs max-w-xs truncate" title={e.description}>{e.description}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {/* Two-column: Active Rules + Terminal Log */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Active Rules */}
                <div className="p-6 rounded-2xl bg-slate-800/30 border border-slate-700/50">
                  <h3 className="text-base font-semibold text-slate-200 mb-4 flex items-center gap-2">
                    <Shield className="w-5 h-5 text-emerald-400" />
                    Active Rules (kept experiments)
                  </h3>
                  <div className="space-y-2">
                    {['NDC_DIAGNOSIS_MISMATCH', 'DUPLICATE_NDC', 'HCC_UPCODING_SUSPECT', 'HIGH_COST_INADEQUATE_DOCUMENTATION',
                      ...allKept.filter(e => e.ruleAdded).map(e => e.ruleAdded)
                    ].map((rule, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-900/40">
                        <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                        <span className="text-xs font-mono text-slate-300">{rule}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="p-6 rounded-2xl bg-slate-900/60 border border-slate-700/50">
                  <h3 className="text-base font-semibold text-slate-200 mb-4 flex items-center gap-2">
                    ...
                    Run Log
                    {arRunning && <span className="flex items-center gap-1 text-xs text-violet-400 font-normal ml-2"><Loader2 className="w-3 h-3 animate-spin" /> running...</span>}
                  </h3>
              ...
              {nextExp && !arRunning && (
                <div className="p-5 rounded-2xl bg-gradient-to-br from-violet-500/10 to-slate-800/50 border border-violet-500/30">
                  <p className="text-xs text-violet-400 font-semibold uppercase tracking-wider mb-2">Next Experiment (queued)</p>
                  <p className="text-slate-200 font-medium">{nextExp.description}</p>
                  <p className="text-slate-400 text-xs mt-1">Rule to add: <span className="font-mono text-violet-300">{nextExp.ruleAdded || 'N/A'}</span></p>
                </div>
              )}

              {!nextExp && (
                <div className="p-5 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-slate-800/50 border border-emerald-500/30 flex items-center gap-4">
